...
module.exports = async function(myPreviousOrder, currentQuote, previousQuote, EMA, executedPositions) {
  ...
  if (Math.floor(currentQuote.lastPrice) >= EMA +1  && previousQuote.lastPrice <= EMA && currentQty <=0) {
  ...
  } else if (Math.ceil(currentQuote.lastPrice)  <= EMA -1 && previousQuote.lastPrice >= EMA && currentQty >= 0) {
    await operations.deleteAllOpen();
